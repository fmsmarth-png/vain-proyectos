...
const Revision: React.FC = () => {
  ...
  useEffect(() => { cargar(); }, []);
  // FIX: useAppFocus solo recarga datos maestros (proyectos, usuario),
  // NO recarga registros para evitar que el scroll vuelva al inicio
  // al recuperar foco después de cerrar un modal de rechazo.
  useAppFocus(() => { cargarSinRegistros(); });
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`registros_${deptoId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'registros', filter: `departamento_id=eq.${deptoId}` }, (payload) => {
        // FIX: en vez de hacer cargarRegistros() completo (que resetea scroll),
        // aplicamos el cambio quirúrgicamente sobre el estado local.
        if (payload.eventType === 'UPDATE') {
          setRegistros(prev => prev.map(r => r.id === payload.new.id ? { ...r, ...payload.new } : r));
        } else if (payload.eventType === 'INSERT') {
          // Para INSERT sí necesitamos los joins (ambientes, partidas, usuarios),
          // así que hacemos fetch solo del registro nuevo.
          supabase
            .from('registros')
            .select('*, ambientes(nombre), partidas(nombre), usuarios!registros_creado_por_fkey(id, nombre)')
            .eq('id', payload.new.id)
            .single()
            .then(({ data }) => {
              if (data) {
                setRegistros(prev => [...prev, data]);
                cache.setRegistros(deptoId, [...(cache.getRegistros(deptoId) ?? []), data]);
              }
            });
  ...
  // FIX: versión de cargar() que NO toca registros ni deptoId,
  // usada por useAppFocus para no interrumpir la sesión de revisión.
  const cargarSinRegistros = async () => {
    if (!online) return;
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      const { data: perfil } = await supabase.from('usuarios').select('*').eq('id', user.id).single();
      if (perfil) { setUsuario(perfil); cache.setUsuario(perfil); }
    } catch {}
  };
  ...
  return (
    <IonPage id="main-content">
      ...
      {/* FIX: ref agregado al IonContent para poder leer/escribir scrollTop */}
      <IonContent ref={contentRef} style={{ '--background': bg }}>
